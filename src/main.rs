...
#[test]
fn test_move_inside_ok() {
    let mut joueur = Joueur {
        nom: "Test".to_string(),
        fruit_de_demon: None,
        position: "L1".to_string(),
        sous_position: "SL1".to_string(),
        inventaire: vec![],
        puissance: 10,
        hp: 100,
    ...
    let sous_lieu1 = SousLieu {
        nom: "Chambre".to_string(),
        position: "L1".to_string(),
        description: "Chambre de repos".to_string(),
        id: "SL1".to_string(),
        connections: vec![Connection {
            orientation: "E".to_string(),
            destination: "SL2".to_string(),
        }],
    };

    let sous_lieu2 = SousLieu {
        nom: "Cuisine".to_string(),
        position: "L1".to_string(),
        description: "Lieu de cuisine".to_string(),
        id: "SL2".to_string(),
        connections: vec![],
    };

    let objets = vec![
        Objet::SousLieu(sous_lieu1.clone()),
        Objet::SousLieu(sous_lieu2.clone()),
    ];

    let result = move_inside(&mut joueur, "E", &objets);
    assert!(result.is_ok());
    assert_eq!(joueur.sous_position, "SL2");
}



#[test]
fn test_move_joueur_ok() {
    let mut joueur = Joueur {
        nom: "Test".to_string(),
        fruit_de_demon: None,
        position: "L1".to_string(),
    ...
    let lieu1 = Lieu {
        id: "L1".to_string(),
        nom: "Village".to_string(),
        description: "Un petit village".to_string(),
        connections: vec![Connection {
            orientation: "N".to_string(),
            destination: "L2".to_string(),
        }],
    ...
    let lieu2 = Lieu {
        id: "L2".to_string(),
        nom: "Forêt".to_string(),
        description: "Forêt dense".to_string(),
        connections: vec![],
        required_key: "".to_string(),
    };

    let sous_lieu2 = SousLieu {
        nom: "Entrée Forêt".to_string(),
        position: "L2".to_string(),
        description: "Entrée de la forêt".to_string(),
        id: "SE1".to_string(),
        connections: vec![],
    };

    let bateau = Objet::ObjetMobile(ObjetMobile {
        nom: "Bateau".to_string(),
        description: "Un bateau solide".to_string(),
        position: "L1".to_string(),
        sous_position: "SL1".to_string(),
        id: "B1".to_string(),
    });
    ...
    let mut objets = vec![
        Objet::Lieu(lieu1),
        Objet::Lieu(lieu2),
        Objet::SousLieu(sous_lieu2),
        bateau,
    ];

    move_joueur(&mut joueur, "N", &mut objets);

    // Le joueur doit avoir changé de lieu
    assert_eq!(joueur.position, "L2");
    assert_eq!(joueur.sous_position, "SE1");

    // Le bateau doit aussi avoir bougé
    let bateau_mis_a_jour = objets.iter().find_map(|o| {
        if let Objet::ObjetMobile(obj) = o {
            if obj.nom == "Bateau" {
    ...
    assert_eq!(bateau_mis_a_jour.position, "L2");
    assert_eq!(bateau_mis_a_jour.sous_position, "SE1");
}
